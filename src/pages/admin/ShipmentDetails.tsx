...
export default function ShipmentDetails() {
    ...
    const isToPayAtDestination = paymentBy === "destinatario";
    ...
    const isFullEditable = isQuote || isCreated;
    const isPartialEditable = !isDelivered;
    const canCollect = !isPaid && (!isToPayAtDestination || isAtOffice);
    const canDelete = isFullEditable;
    ...
    const handleDeleteShipment = async () => {
        if (!shipment) return;
        const confirm = window.confirm("¿Estás seguro de ELIMINAR físicamente esta encomienda? Esta acción no se puede deshacer.");
        if (!confirm) return;

        try {
            await ENV.delete(`/shipments/${shipment.id}`);
        ...
        <div className="space-y-6 max-w-5xl mx-auto pb-12">
            ...
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                ...
                <div className="flex gap-2 shrink-0 overflow-x-auto pb-2 md:pb-0">
                    {canDelete && (
                        <Button variant="outline" className="gap-2 h-11 sm:h-12 border-red-200 text-red-600 hover:bg-red-50" onClick={handleDeleteShipment}>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="space-y-5">
                    ...
                    <div className="border border-border/60 rounded-2xl p-4 sm:p-5 bg-card/60 backdrop-blur-sm shadow-sm">
                        ...
                        <div className="flex gap-2 mb-4">
                            <ShipmentTypeCard disabled={!isFullEditable} active={type === "paquete"} icon={Package} label="Paquete" description="Caja o bulto" onClick={() => setType("paquete")} />
                            <ShipmentTypeCard disabled={!isFullEditable} active={type === "sobre"} icon={Mail} label="Sobre" description="Documento" onClick={() => setType("sobre")} />
                        </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div className="space-y-2">
                                ...
                                <Select value={originId} onValueChange={setOriginId} disabled={!isFullEditable}>
                            ...
                            <div className="space-y-2">
                                ...
                                <Select value={destinationId} onValueChange={setDestinationId} disabled={!isFullEditable}>
                                    <SelectTrigger className="h-11">
                        ...
                        <div className="p-4 sm:p-5">
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                ...
                                <div className="space-y-1.5">
                                    ...
                                    <Input disabled={!isFullEditable} type="number" min="0" step="0.1" value={weight} onChange={(e) => setWeight(e.target.value)} className="h-11 border-primary/50" placeholder="Ej: 5" />
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="border border-border/60 rounded-2xl p-4 sm:p-5 bg-card/60 shadow-sm">
                        ...
                        <div className="mb-4">
                            ...
                            <div className="flex gap-3">
                                <TransportCard disabled={!isFullEditable} active={transport === "terrestre"} icon={Truck} label="Terrestre" subtitle="Camión/Flota" onClick={() => setTransport("terrestre")} />
                                <TransportCard disabled={!isFullEditable} active={transport === "aereo"} icon={Plane} label="Aéreo" subtitle="Avión" onClick={() => setTransport("aereo")} />
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-3 gap-2 sm:gap-3">
                                {SERVICE_TIERS.map((tier) => (
                                    <button disabled={!isFullEditable} key={tier.id} type="button" onClick={() => setService(tier.id as any)} className={cn("relative flex flex-col gap-2 p-2 rounded-xl border-2 text-left transition-all duration-300", service === tier.id ? `bg-gradient-to-br ${tier.bgGradient} ${tier.borderColor} shadow-md` : "border-border/70 bg-card hover:bg-muted/30", !isFullEditable && "opacity-50 pointer-events-none")}>
                    ...
                    <div className="relative rounded-2xl overflow-hidden border border-primary/30 shadow-lg shadow-primary/10">
                        ...
                        <div className="relative p-4 sm:p-6 flex flex-col sm:flex-row items-center justify-between gap-4">
                            <div>
                                ...
                                <Button disabled={!isFullEditable} variant="outline" size="sm" onClick={handleCalculatePrice} className="mt-2 text-xs h-8">
                            ...
                            <div className="flex flex-col items-end gap-2 shrink-0">
                                <div className="flex items-center gap-2">
                                    ...
                                    <Input disabled={!isFullEditable} type="number" step="0.5" value={discount} onChange={e => setDiscount(e.target.value)} className="h-8 w-20 text-right" />
                                </div>
                                <div className="flex items-center gap-2 mt-1 border-t border-border pt-1">
                                    ...
                                    <Input disabled={!isFullEditable} type="number" step="0.5" value={price} onChange={e => setPrice(e.target.value)} className="h-10 w-28 text-right font-black text-xl text-primary" />
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div className="space-y-5">
                    ...
                    <div className="border border-border/60 rounded-2xl p-4 sm:p-5 bg-card/60 shadow-sm">
                        ...
                        <div className="grid grid-cols-1 gap-4">
                            <div className="space-y-2">
                                ...
                                <Select value={status} onValueChange={(val: ShipmentStatus) => setStatus(val)} disabled={isDelivered || !receiverCi.trim() || !receiverName.trim() || !receiverPhone.trim()}>
                                    <SelectTrigger className="h-11">
                                        <SelectValue />
                                    </SelectTrigger>
                                    <SelectContent>
                                        ...
                                        <SelectItem value="delivered" disabled={!isPaid}>
                                            Entregado {!isPaid && "(Requiere Pago)"}
                                        </SelectItem>
                                    </SelectContent>
                                </Select>
                            ...
                            <div className="space-y-2">
                                ...
                                <Textarea disabled={isDelivered} value={observation} onChange={(e) => setObservation(e.target.value)} className="resize-none min-h-[60px]" />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="border border-border/60 rounded-2xl p-4 sm:p-5 bg-card/60 shadow-sm space-y-4">
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="flex gap-2">
                                <Input disabled={!isFullEditable} value={senderCi} onChange={e => setSenderCi(e.target.value)} placeholder="Buscar por CI..." className={!senderCi.trim() ? "border-destructive/50" : ""} />
                                <Button disabled={!isFullEditable || isSearchingSender || senderCi.length < 5} type="button" variant="outline" size="icon" className="shrink-0 h-10 w-10 text-primary hover:text-primary transition-colors hover:bg-primary/10" onClick={() => handleSearchClient('sender')}>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <div className="space-y-2">
                                ...
                                <Input disabled={!isFullEditable} value={senderName} onChange={e => setSenderName(e.target.value)} className={!senderName.trim() ? "border-destructive/50" : ""} />
                            </div>
                            <div className="space-y-2">
                                <Label>Teléfono <span className="text-destructive">*</span></Label>
                                <Input disabled={!isFullEditable} value={senderPhone} onChange={e => setSenderPhone(e.target.value)} className={!senderPhone.trim() ? "border-destructive/50" : ""} />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="border border-border/60 rounded-2xl p-4 sm:p-5 bg-card/60 shadow-sm space-y-5">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <button disabled={!isFullEditable} type="button" onClick={() => setPaymentBy("remitente")} className={cn("flex flex-col items-center justify-center p-3 rounded-xl border-2 transition-all", paymentBy === "remitente" ? "border-emerald-500 bg-emerald-500/10 text-emerald-600" : "border-border bg-card", !isFullEditable && "opacity-50 pointer-events-none")}>
                                <Wallet className="h-5 w-5 mb-1" />
                                <span className="text-xs font-bold">Origen</span>
                            </button>
                            <button disabled={!isFullEditable} type="button" onClick={() => setPaymentBy("destinatario")} className={cn("flex flex-col items-center justify-center p-3 rounded-xl border-2 transition-all", paymentBy === "destinatario" ? "border-sky-500 bg-sky-500/10 text-sky-600" : "border-border bg-card", !isFullEditable && "opacity-50 pointer-events-none")}>
                                <MapPinOff className="h-5 w-5 mb-1" />
                                <span className="text-xs font-bold">Destino</span>
                            </button>
                        </div>

                        <div className={cn("flex items-center space-x-2 pt-2 border-t border-border", isFullEditable ? "cursor-pointer" : "opacity-50 pointer-events-none")} onClick={() => isFullEditable && setIsFragile(!isFragile)}>
